...
import { Plus, Pencil, Trash2, Truck, Phone, FileText, RefreshCcw, Loader2 } from 'lucide-react';
...
export default function ProveedoresList() {
  ...
  const [formData, setFormData] = useState({ nombre: '', ruc: '', telefono: '', direccion: '' });
  ...
  const toggleSelectAll = () => setSelected(selected.length === proveedores.length ? [] : proveedores.map(p => p.id));
  const allSelected = proveedores.length > 0 && selected.length === proveedores.length;

  const handleBulkDelete = async () => {
    if (!window.confirm(`¿Eliminar ${selected.length} proveedor(es)?`)) return;
    setIsBulkDeleting(true);
    try {
      const res = await api.delete('/api/proveedores/bulk', { data: { ids: selected } });
  ...
  const handleEdit = (proveedor) => {
    ...
    setFormData({ nombre: proveedor.nombre, ruc: proveedor.ruc || '', telefono: proveedor.telefono || '', direccion: proveedor.direccion || '' });
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" size="icon" className="h-10 w-10 rounded-sm border-border text-muted-foreground bg-card hover:bg-muted" onClick={() => fetchProveedores()} disabled={isLoading}>
              <RefreshCcw className={`h-4 w-4 ${isLoading ? 'animate-spin' : ''}`} />
            </Button>
            <Can permission="crear.proveedor">
              <Button className="rounded-sm shadow-sm" onClick={() => { resetForm(); setIsDialogOpen(true); }}>
            ...
            <TableBody className="divide-y divide-border/40">
              {isLoading ? (
                ...
                proveedores.map((prov) => (
                  <TableRow key={prov.id} className={`hover:bg-muted/30 transition-colors group ${selected.includes(prov.id) ? 'bg-primary/5' : ''}`}>
                    <TableCell className="px-4">
                      <Checkbox checked={selected.includes(prov.id)} onCheckedChange={() => toggleSelect(prov.id)} />
                    </TableCell>
                    ...
                    <TableCell className="text-right px-6">
                      <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Can permission="editar.proveedor">
                          <Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-foreground hover:bg-muted" onClick={() => handleEdit(prov)}>
                            <Pencil className="h-4 w-4" />
                          </Button>
                        </Can>
                        <Can permission="eliminar.proveedor">
                          <Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={() => handleDelete(prov.id)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-lg bg-card">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-foreground">{editingProveedor ? 'Modificar Registro' : 'Nuevo Socio'}</DialogTitle>
            <DialogDescription className="text-muted-foreground">Ficha maestra de proveedor del negocio</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-6 pt-4">
            <div className="space-y-2">
              ...
              <Input placeholder="Ej. DISTRIBUIDORA CENTRAL S.A." value={formData.nombre} onChange={(e) => setFormData({ ...formData, nombre: e.target.value })} className="h-10 rounded-sm border-border bg-background" required />
            </div>
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground ml-1">RUC / ID TRIBUTARIO</label>
                <Input placeholder="000-000-000" value={formData.ruc} onChange={(e) => setFormData({ ...formData, ruc: e.target.value })} className="h-10 rounded-sm border-border bg-background" />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground ml-1">Teléfono Directo</label>
                <Input placeholder="555-0000" value={formData.telefono} onChange={(e) => setFormData({ ...formData, telefono: e.target.value })} className="h-10 rounded-sm border-border bg-background" />
              </div>
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground ml-1">Dirección Operativa</label>
              <Input placeholder="Calle comercial #123" value={formData.direccion} onChange={(e) => setFormData({ ...formData, direccion: e.target.value })} className="h-10 rounded-sm border-border bg-background" />
            </div>
            <DialogFooter className="pt-6 border-t border-border/50 gap-2">
              <Button type="button" variant="ghost" className="text-muted-foreground hover:bg-muted" onClick={() => setIsDialogOpen(false)} disabled={isSubmitting}>Cancelar</Button>
              <Button type="submit" className="min-w-[120px] rounded-sm font-bold shadow-sm" disabled={isSubmitting}>
                {isSubmitting ? <><RefreshCcw className="mr-2 h-4 w-4 animate-spin" />{editingProveedor ? 'Actualizando...' : 'Guardando...'}</> : (editingProveedor ? 'Actualizar Ficha' : 'Vincular Socio')}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
