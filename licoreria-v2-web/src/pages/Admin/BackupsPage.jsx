  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-1000">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between px-1">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Respaldos</h1>
          <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest mt-1 opacity-70">Copias de seguridad de tu base de datos</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card className="border-none shadow-sm bg-white overflow-hidden hover:shadow-md transition-all duration-300 rounded-sm group">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-4">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Estado</span>
            <div className={`p-2 rounded-sm ${last ? 'bg-emerald-500/10' : 'bg-amber-500/10'}`}>
              {last ? <CheckCircle size={18} className="text-emerald-600" weight="duotone" /> : <WarningCircle size={18} className="text-amber-600" weight="duotone" />}
            </div>
          </CardHeader>
          <CardContent className="pb-5">
            <div className={`text-xl font-black tracking-tight leading-none mb-1 ${last ? 'text-emerald-600' : 'text-amber-600'}`}>
        ...
        <Card className="border-none shadow-sm bg-white overflow-hidden hover:shadow-md transition-all duration-300 rounded-sm group">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-4">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Último Respaldo</span>
            <div className="p-2 rounded-sm bg-primary/10">
              <Clock size={18} className="text-primary" weight="duotone" />
            </div>
          </CardHeader>
          <CardContent className="pb-5">
            <div className="text-xl font-black tracking-tight text-slate-900 leading-none mb-1">
        ...
        <Card className="border-none shadow-sm bg-white overflow-hidden hover:shadow-md transition-all duration-300 rounded-sm group">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-1 pt-4">
            ...
            <div className="p-2 rounded-sm bg-violet-500/10">
              <Database size={18} className="text-violet-600" weight="duotone" />
            </div>
          </CardHeader>
          <CardContent className="pb-5">
      ...
      <Card className="border-none shadow-sm bg-white overflow-hidden rounded-sm">
        <CardHeader className="flex flex-row items-center justify-between border-b border-slate-50 pb-4 px-6 pt-6">
          <div>
            <h2 className="text-base font-bold text-slate-800">Historial de Respaldos</h2>
            <p className="text-xs text-slate-500 font-medium">Archivos disponibles para descarga y restauración.</p>
          </div>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-16 text-center">
              ...
              <p className="text-xs font-medium text-slate-400">Crea tu primer respaldo para proteger los datos de tu negocio.</p>
            </div>
          ) : (
                    ...
                    <tr key={b.path} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs font-semibold text-slate-700 truncate max-w-[200px]">{b.filename}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[10px] font-bold text-slate-500 bg-slate-50 px-2 py-0.5 rounded-sm">{b.size_human}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div>
      ...
      <Dialog open={confirmDialog.open} onOpenChange={(open) => setConfirmDialog({ ...confirmDialog, open })}>
        <DialogContent aria-describedby={undefined}>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {confirmDialog.type === 'restore' && (
                <>
                  Se restaurará <span className="font-bold text-slate-800">{confirmDialog.filename}</span>.
                  <span className="block mt-2 text-amber-600 font-semibold">⚠ Esto reemplazará TODOS los datos actuales con los del respaldo. Los cambios realizados después de esta copia se perderán.</span>
                </>
              )}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            {confirmDialog.type === 'restore' && (
              <Button onClick={() => handleRestore(confirmDialog.path)} className="rounded-sm bg-amber-600 hover:bg-amber-700">
