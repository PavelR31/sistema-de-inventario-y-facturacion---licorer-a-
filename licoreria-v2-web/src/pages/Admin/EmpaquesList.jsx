...
import { Plus, Pencil, Trash2, Package, RefreshCcw, Layers } from 'lucide-react';
      ...
      {viewMode === 'table' ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {isLoading ? (
            ...
            <Card key={emp.id} className="border shadow-sm bg-card group p-5 hover:shadow-md transition-all">
              ...
              <p className="text-[11px] font-bold text-muted-foreground mt-1 line-clamp-2 min-h-[32px] uppercase tracking-wider">
                {emp.cantidad_unidades} Unidades Totales
              </p>
            </Card>
          ))}
        </div>
        ...
        <DialogContent className="max-w-md bg-card">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-foreground">
              {editingEmpaque ? 'Editar Empaque' : 'Nuevo Empaque'}
            </DialogTitle>
            <DialogDescription className="text-muted-foreground">
              Define cómo vendes o almacenas tus productos (Ej. Caja, Six-pack, etc.)
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-5 pt-4">
            <div className="space-y-2">
              ...
              <Input 
                placeholder="Ej. Caja, Paca, Botella"
              ...
              <Button type="submit" className="min-w-[120px] rounded-sm font-bold shadow-sm" disabled={isSubmitting}>
                {isSubmitting ? (
                  <>
                    ...
                    {editingEmpaque ? 'Actualizando...' : 'Creando...'}
                  </>
                ) : (
                  editingEmpaque ? 'Actualizar' : 'Crear'
                )}
              </Button>
            </DialogFooter>
          </form>
